...
class GroupLessonController extends GroupContentController {
  ...
  public function addPage(GroupInterface $group, $create_mode = FALSE) {
    ...
    $storage_handler = $this->entityTypeManager->getStorage('ejikznayka_lesson');
    foreach ($this->addPageBundles($group, $create_mode) as $plugin_id => $bundle_name) {
      if (!empty($build['#bundles'][$bundle_name])) {
        ...
        $t_args = ['%lesson_type' => $bundle_label];
        $description = $create_mode
          ? $this->t('Create a lesson of type %lesson_type in the group.', $t_args)
          : $this->t('Add an existing lesson of type %lesson_type to the group.', $t_args);
  ...
  protected function addPageBundles(GroupInterface $group, $create_mode) {
    ...
    foreach ($plugin_ids as $key => $plugin_id) {
      if (strpos($plugin_id, 'group_lesson:') !== 0) {
